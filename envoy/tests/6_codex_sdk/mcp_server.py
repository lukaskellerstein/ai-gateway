"""An MCP server in a process of its own, spoken to over stdio. 04_mcp.py runs it.

NOT A TEST, AND THE RUNNER SKIPS IT: `run_all.py` globs `NN_*.py`, so this name
keeps it out of the suite while `04_mcp.py` has Codex start it as a child.

IT WRITES A MARKER FILE ON STARTUP, and that marker is what 04 asserts on —
proof that Codex read the `mcp_servers` config, resolved the command and got a
