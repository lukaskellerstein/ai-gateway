...
(LiteLLM stops at 4095 on the same prompt, off its stored route default. Both
projects carry this same script and each asserts its own table, which is the
point.)
...
def check_model_listing(gateway: Gateway, _model: str) -> str:
    ...
    check(
        ...
        "../../config/<engine>.yaml, so a 404 here means the rules did not load.",
    )
