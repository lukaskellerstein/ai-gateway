...
`common.Gateway` makes about HOW to call it. THREE OF THEM ARE `False`, and this
script exists to prove they are still false — an absence nobody checks is an
absence somebody eventually assumes away.

    property                Envoy 26000, and what is asserted here
    ----------------------  -------------------------------------------------
    checks_api_key   False  a bogus Bearer token gets 200. `aigw run` has no
                            caller authentication at all
    lists_models     True   GET /models returns the alias list, built from the
                            AIGatewayRoute rules. THE ONE THAT IS TRUE
    echoes_alias     False  response.model is the ENGINE'S own model id —
                            modelNameOverride rewrote it and nothing undoes that
    exposes_route_limits
                     False  there is no /model/info route, and a route rule
                            carries a timeout but no token ceiling
...
(LiteLLM stops at 4095 on the same prompt, off its stored route default; MLflow
runs to 13961. All three projects carry this same script and each asserts its own
...
def _root(base_url: str) -> str:
    ...
    Dropping the trailing `/v1` gives `http://localhost:26000`, which is where
    `/model/info` WOULD live — derived rather than written out a second time. It
    404s here, and that is the assertion.
    """
...
def check_api_key(gateway: Gateway, model: str) -> str:
    ...
    check(
        ...
        f"HTTP {status}. 200 is correct here — `aigw run` authenticates no caller at all "
        "and never reads this header. A 401 would mean something in front of it started "
        "checking. The key that matters is the one sent UPSTREAM, from a Secret in "
        "../config/<engine>.yaml.",
    )
...
def check_model_listing(gateway: Gateway, _model: str) -> str:
    ...
    check(
        ...
        f"HTTP {status}. This gateway builds the list from the AIGatewayRoute rules in "
        "../config/<engine>.yaml, so a 404 here means the rules did not load — not that the "
        "route is absent by design, which is what it means on MLflow.",
    )
