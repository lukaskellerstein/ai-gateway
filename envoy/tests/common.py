...
24000 or 25000. If you want that, call the other ports by hand.
...
like LiteLLM does and checks no key like MLflow does, so a test that assumed
"LiteLLM or not-LiteLLM" would be wrong about it. That is why each project
declares its own table.
"""
...
# ./.env names ONE engine, and the aliases of every other engine have no route
# rule at all — so a fixed `lms-4b` default would 404 on a healthy gateway.
#
...
# needs minutes before its first token — the same fact that sets `request: 60m`
# on both the route and the backend in ../config/<engine>.yaml. Retries are off
...
class Gateway:
    ...
    lists_models
        ...
        AIGatewayRoute rules. THIS IS THE LINE THAT DIFFERS FROM MLFLOW, which
        404s the same route.
    echoes_alias
...
#   (LiteLLM, for contrast, stopped at 4095 on its stored route default, and
#    MLflow ran to 13961 on the same prompt. Measured here 2026-09-04.)
#
...
GATEWAY = Gateway(
    name="envoy",
    base_url="http://localhost:26000/v1",
