...
THIRD word in `.env` says otherwise:

    COMPOSE_PROFILES    litellm | mlflow | litellm,mlflow | all      WHICH GATEWAY
...
here runs, LiteLLM reads `litellm/<engine>.yaml` exactly as before, and the MLflow
seed writes exactly the endpoints `mlflow/<engine>.py` names. Those hand-written
files are the documentation for how to configure this gateway by hand, so they stay.

WITH IT ON, DISCOVERY IS ADDITIVE, NEVER A REPLACEMENT. The generated config
INCLUDES the hand-written one:

    litellm/discovered-lms.yaml
