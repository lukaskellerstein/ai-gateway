...
25000. If you want that, call both ports by hand.
...
# config at all — so a fixed `lms-4b` default would fail with "model not found" on a
...
# `timeout: 3600` on every chat route in ../config/. Retries are off because a test
# that silently retries hides the failure it exists to find.
REQUEST_TIMEOUT_SECONDS = 3600.0

# The allowance a scenario sends when it sends one at all. It has to clear a
# REASONING block: both `unsloth-*` chat routes, both `ollama-*` ones and `lms-4b`
# spend this budget on thinking before they write a word. A model that runs out
# mid-thought returns EMPTY content with `finish_reason: "length"` and raises
...
#
# ON THIS GATEWAY IT IS USUALLY UNUSED — see `body_extras` below.
MAX_TOKENS = 2048
...
# `body_extras` is EMPTY here rather than a copy of MAX_TOKENS.
#
